...
def job_config = [
        job: [
                name: "nowy_projekt2410_Pipeline_develop"
        ],
        git: [
                branch: "feature-1666597656284"
        ]
]
...
def customLib = library identifier: 'nowy_projekt2410_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/jarro06/nowy_projekt2410_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.nowy_projekt2410.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "nowy_projekt2410_Pipeline_develop", "feature-1666597656284")
                }
            }    
        }
        stage('Build') {
